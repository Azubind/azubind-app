...
async function saveAdminProcess(id,button){const card=button.closest(".admin-card"),note=card.querySelector(".admin-note"),interview=card.querySelector(".admin-interview"),msg=card.querySelector(".admin-process-message");button.disabled=true;msg.textContent="Kaydediliyor...";const now=new Date().toISOString(),payload={admin_note:note.value.trim()||null,interview_at:interview.value?new Date(interview.value).toISOString():null,candidate_notification:interview.value?"Görüşme tarihin güncellendi.":"Başvuru sürecinde yeni bir güncelleme var.",candidate_notification_at:now,updated_at:now};const {error}=await sb.from("applications").update(payload).eq("id",id);button.disabled=false;msg.textContent=error?"Kaydedilemedi: "+error.message:"Kaydedildi ✓";}
function safeText(v){const d=document.createElement("div");d.textContent=String(v);return d.innerHTML;}
async function saveAdminStatus(id,select){const note=select.parentElement.querySelector("em");select.disabled=true;note.textContent="Kaydediliyor...";const now=new Date().toISOString(),labels={pending:"Başvurun alındı.",reviewing:"Başvurun inceleniyor.",contacted:"Azubind seninle iletişime geçme aşamasında.",interview:"Başvurun görüşme aşamasına geçti.",contract:"Başvurun sözleşme aşamasına geçti.",completed:"Başvuru sürecin tamamlandı.",rejected:"Başvuru sürecin sonuçlandı."};const {error}=await sb.from("applications").update({status:select.value,candidate_notification:labels[select.value]||"Başvuru durumun güncellendi.",candidate_notification_at:now,updated_at:now}).eq("id",id);select.disabled=false;note.textContent=error?"Kaydedilemedi":"Kaydedildi ✓";}
