...
async function refreshAuth(){if(!sb)return;const {data:{user}}=await sb.auth.getUser();const guest=document.getElementById("auth-guest"),logged=document.getElementById("auth-user"),email=document.getElementById("user-email"),account=document.getElementById("account-button");if(guest)guest.hidden=!!user;if(logged)logged.hidden=!user;if(email&&user)email.textContent=user.email;if(account)account.textContent=user?"Hesabım":"Giriş Yap";await checkAdmin(user);await loadApplication(user);}
