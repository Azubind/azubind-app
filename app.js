...
function filterAdminSearch(){const input=document.getElementById("admin-search");if(!input)return;const q=input.value.trim().toLocaleLowerCase("tr-TR");document.querySelectorAll("#admin-list .admin-card").forEach(card=>{const haystack=card.textContent.toLocaleLowerCase("tr-TR");card.hidden=!!q&&!haystack.includes(q);});}
