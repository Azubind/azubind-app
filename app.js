...
const loginForm=document.getElementById("login-form");if(loginForm)loginForm.addEventListener("submit",async e=>{e.preventDefault();const f=new FormData(loginForm);const {error}=await sb.auth.signInWithPassword({email:f.get("email"),password:f.get("password")});if(error)showAuthMessage("Giriş yapılamadı: "+error.message);else{showAuthMessage("Giriş başarılı.");await refreshAuth();}});
const forgotPassword=document.getElementById("forgot-password");if(forgotPassword)forgotPassword.addEventListener("click",async()=>{const email=loginForm&&loginForm.elements.email?loginForm.elements.email.value.trim():"";if(!email){showAuthMessage("Önce e-posta adresini yaz.");return;}forgotPassword.disabled=true;showAuthMessage("Şifre sıfırlama bağlantısı gönderiliyor...");const {error}=await sb.auth.resetPasswordForEmail(email,{redirectTo:"https://app.azubind.com"});forgotPassword.disabled=false;showAuthMessage(error?"Bağlantı gönderilemedi: "+error.message:"Şifre sıfırlama bağlantısı e-posta adresine gönderildi.");});
const signupForm=document.getElementById("signup-form");if(signupForm)signupForm.addEventListener("submit",async e=>{e.preventDefault();const f=new FormData(signupForm);const {error}=await sb.auth.signUp({email:f.get("email"),password:f.get("password"),options:{emailRedirectTo:"https://app.azubind.com"}});showAuthMessage(error?"Kayıt oluşturulamadı: "+error.message:"Kayıt oluşturuldu. E-postana gelen doğrulama bağlantısına tıkla.");});
...
if(resetPasswordForm)resetPasswordForm.addEventListener("submit",async e=>{e.preventDefault();const msg=document.getElementById("reset-password-message"),f=new FormData(resetPasswordForm),password=String(f.get("password")||""),confirmPassword=String(f.get("passwordConfirm")||"");msg.hidden=false;if(password.length<6){msg.textContent="Şifre en az 6 karakter olmalı.";return;}if(password!==confirmPassword){msg.textContent="Şifreler eşleşmiyor.";return;}const button=resetPasswordForm.querySelector('button[type="submit"]');button.disabled=true;msg.textContent="Şifre güncelleniyor...";const {error}=await sb.auth.updateUser({password});button.disabled=false;if(error){msg.textContent="Şifre güncellenemedi: "+error.message;return;}msg.textContent="Şifren başarıyla güncellendi ✓";setTimeout(()=>{document.body.dataset.passwordRecovery="0";window.history.replaceState({},document.title,window.location.pathname);refreshAuth();},800);});
if(sb){
