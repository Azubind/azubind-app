...
async function saveAdminStatus(id,select){const note=select.parentElement.querySelector("em");select.disabled=true;note.textContent="Kaydediliyor...";const now=new Date().toISOString(),labels={pending:"Başvurun alındı.",reviewing:"Senin için uygun işverenler araştırılıyor.",contacted:"Azubind seninle iletişime geçme aşamasında.",interview:"Başvurun görüşme aşamasına geçti.",contract:"Başvurun sözleşme aşamasına geçti.",completed:"Başvuru sürecin tamamlandı.",rejected:"Başvuru sürecin sonuçlandı."};const {error}=await sb.from("applications").update({status:select.value,candidate_notification:labels[select.value]||"Başvuru durumun güncellendi.",candidate_notification_at:now,candidate_notification_read_at:null,updated_at:now}).eq("id",id);select.disabled=false;note.textContent=error?"Kaydedilemedi":"Kaydedildi ✓";}
