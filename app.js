...
async function loadAdminPanel(){
 ...
 const setStat=(id,n)=>{const el=document.getElementById(id);if(el)el.textContent=n;};setStat("stat-total",data.length);setStat("stat-reviewing",data.filter(x=>x.status==="reviewing").length);setStat("stat-interview",data.filter(x=>x.status==="interview").length);setStat("stat-contract",data.filter(x=>x.status==="contract").length);setStat("stat-completed",data.filter(x=>x.status==="completed").length);
