...
async function saveAdminProcess(id,button){const card=button.closest(".admin-card"),note=card.querySelector(".admin-note"),interview=card.querySelector(".admin-interview"),msg=card.querySelector(".admin-process-message");button.disabled=true;msg.textContent="Kaydediliyor...";const payload={admin_note:note.value.trim()||null,interview_at:interview.value?new Date(interview.value).toISOString():null,updated_at:new Date().toISOString()};const {error}=await sb.from("applications").update(payload).eq("id",id);button.disabled=false;msg.textContent=error?"Kaydedilemedi: "+error.message:"Kaydedildi ✓";}
